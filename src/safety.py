...
STOPWORDS = {
    ...
    "error", "code", "codes", "regarding", "fault", "troubleshoot", "fix", "issue", "problem",
    "at", "by", "if", "my", "our", "there", "their", "when", "where", "which"
}
...
# Technical vocabulary covering plant hardware, parts, telemetry, and fault symptoms
...
def extract_candidate_error_codes(text: str) -> List[str]:
    ...
    for m in alpha_matches:
        # Exclude common machine model names like MX-7 or P400/X200 if needed, but keep error codes
...
def detect_foreign_machine(query: str, extra_known_machines: Optional[List[str]] = None) -> Tuple[bool, Optional[str]]:
    ...
    for target in prep_matches:
        ...
        if not target_tokens:
            continue
        # If target has an equipment noun or noun phrase
    ...
    for dft in direct_foreign_terms:
        if re.search(rf"\b{re.escape(dft)}\b", query_lower):
            # Verify if this term is in known aliases
...
def _extract_content_tokens(text: str) -> List[str]:
    ...
    tokens = []
    for t in raw_tokens:
        if t in STOPWORDS or t in ALL_KNOWN_MACHINE_ALIASES:
            continue
        tokens.append(t)
        if "-" in t:
            sub = [s for s in t.split("-") if s and len(s) > 1 and s not in STOPWORDS and s not in ALL_KNOWN_MACHINE_ALIASES]
            tokens.extend(sub)
    return list(dict.fromkeys(tokens))


def is_sufficient(retrieved_chunks: List[Dict[str, Any]], query: str = "", threshold: float = 0.35) -> Tuple[bool, Any]:
    """
    Evaluates whether retrieved chunks provide sufficient, ground-truth information to answer.

    Strict Ordered Gate Architecture:
    Gate 0: Check presence of chunks.
    Gate 1: Error Code Verification (T2/T3 vs T4)
            - If query mentions candidate error codes, at least ONE must exist in the manual chunks/metadata.
            - If none exist -> REFUSE immediately (catches fake codes, e.g. A999 on RobotArm-300, error 999).
            - If code is confirmed in chunks -> ACCEPT (T4).
    Gate 2: Foreign Machine Inversion Check (T5)
            - Queries specifying unindexed plant machines (e.g. forklift, laser cutter) -> REFUSE.
    Gate 3: Undocumented Safety Bypasses & Unauthorized Actions -> REFUSE.
    Gate 4: Undocumented Signaling Indicators (LED / Blink patterns) -> REFUSE.
    Gate 5: General Symptoms & Borderline Overlap:
            - Zero domain tokens and raw_sim < 0.40 -> REFUSE (T1, pure noise / gibberish).
            - raw_sim < 0.35 -> REFUSE (below general symptom floor).
            - 0.35 <= raw_sim < 0.50 -> Requires >= 40% query token overlap in chunk (T6).
            - raw_sim >= 0.50 with domain vocabulary -> ACCEPT (T7).
    ...
    for c in retrieved_chunks:
        ...
        # Also extract any normalized codes found in chunk text
