...
class MultilingualTranslationModule:
    ...
    def translate_input(self, text: str) -> Dict[str, Any]:
        ...
        # Step 4: Prioritize Domain Semantic Engine for deterministic factory floor terms
        if not translated_text:
            domain_trans = self._fallback_domain_translate(cleaned, lang_name)
            if domain_trans and domain_trans != cleaned:
                translated_text = domain_trans

        # Step 5: Fallback to Gemini translation for open-ended queries
        if not translated_text:
